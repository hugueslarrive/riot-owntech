...
/* functions from cpu/stm32/periph/spi.c */
...
int main(void)
{
    ...
    printf("CR1: %lx\n", spi_config[0].dev->CR1);
    printf("CR2: %lx\n", spi_config[0].dev->CR2);
    printf("SPI_CR1_MSTR: %lu\n", SPI_CR1_MSTR);
    
    /* unset MSTR bit (switch to slave mode) */
    spi_config[0].dev->CR1 &= ~SPI_CR1_MSTR;
    ...
    while(1) {
        memset(buf, 0, sizeof(buf));
        spi_config[0].dev->CR1 |= (SPI_CR1_SPE);
        _transfer_no_dma(spiconf.dev, "ready", buf, 5);
        spi_config[0].dev->CR1 &= ~(SPI_CR1_SPE);
        print_bytes("Received bytes", buf, 5);
        printf("\"%s\" received\n", buf);
    }
